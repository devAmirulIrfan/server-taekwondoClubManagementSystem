...
async function getAllStudent() {
    ...
    SELECT s.id, s.studentName, s.birthDate, c.centerName, s.gradeId, g.gradeName, p.parentName, p.centerId, p.contactNo, st.statusName, st.id As ststusId
        FROM student AS s
...
async function updateStudent(studentName, birthDate, gradeId, parentId, statusId, studentId) {
    await pool.execute(
      `
      UPDATE student
      SET studentName = ?, birthDate = ?, gradeId = ?, parentId = ?, statusId = ?
      WHERE id = ?
      `,
